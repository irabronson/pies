  ...
  const videoPausePlay = new IntersectionObserver(
    entries => {
      ...
      if (entry.isIntersecting) {
				// Adding a timeout to address the 'play() request was interrupted by a new load request' error which seems to be a result of the video file(s) not being loaded at the exact time of the play() function and thus cannot start immediately.
				// If playback succeeds, the Promise is fulfilled and the play() event is fired at the same time. If playback fails, the Promise is rejected along with the error.
				// This is likely due to being tied to an IO for play/pause and already being visible in the viewport, and/or related to a race between play() and pause() which are asynchronous.
        setTimeout(() => {
	  			entry.target.play();
        }, 100);
...
document.querySelectorAll('.qr-icon-container').forEach(iconContainer => {
  iconContainer.addEventListener('click', (event) => {
    const qrContainer = event.target.closest('.qr-container');
    if (qrContainer) {
